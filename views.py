...
from locales import THEME, STATUSES, MEDIA_TYPES, SORT_KEYS
...
class AddEditDialog:
    ...
    def _build_form(self):
        ...
        for r, (label_text, widget) in enumerate(rows):
            ...
            lbl.grid(row=r, column=0, sticky="w", padx=(0, 10), pady=5)
    ...
    def _build_buttons(self):
        ...
        self._bind_hover(self.btn_save)
    ...
    def _populate(self, data: dict):
        ...
        self.year_var.set(str(data.get("year", 2024)))
    ...
    def _on_save(self):
        ...
        ok, err = self.controller.validate_item_form(data)
        if not ok:
...
class FilterView:
    ...
    def _build(self):
        ...
        self.type_cb = ttk.Combobox(
            ...
            values=[""] + MEDIA_TYPES, state="readonly", width=10,
        )
        ...
        self.genre_entry.grid(row=0, column=5, sticky="w", **pad)
...
class StatsView:
    ...
    def _build(self):
        ...
        for i, (status, color_key) in enumerate(self._status_color_keys.items()):
            name_lbl = tk.Label(f, text=status, bg=THEME["bg"], fg=THEME[color_key],
                                font=("Segoe UI", 10), width=16, anchor="w")
            name_lbl.grid(row=7 + i, column=0, sticky="w", pady=2)
            cnt = tk.Label(f, text="0", bg=THEME["bg"], fg=THEME[color_key],
...
class AnalyticsView:
    ...
    def _build(self):
        ...
        self.lbl_top5 = tk.Label(f, text=self.controller.t("analytics_top5"),
                                 bg=THEME["bg"], fg=THEME["text"],
                                 font=("Segoe UI", 14, "bold"))
        ...
        self.top5_tree.column("title",  width=300, anchor="w",      minwidth=120)
    ...
    def update_analytics(self, data: dict):
        ...
        for i, d in enumerate(data.get("top5_rated", []), start=1):
            rating = d.get("rating")
...
class MainView:
    ...
    def _build_collection_tab(self):
        ...
        self._bind_hover(self.btn_lang)
        ...
        self.filter_view = FilterView(f, self.controller)
        ...
        self.tree.column("title",  width=200, anchor="w",      minwidth=100)
        self.tree.column("type",   width=70,  anchor="center", minwidth=55)
        self.tree.column("genre",  width=100, anchor="w",      minwidth=65)
        self.tree.column("year",   width=75,  anchor="center", minwidth=60)
        self.tree.column("rating", width=60,  anchor="center", minwidth=50)
        self.tree.column("status", width=120, anchor="center", minwidth=85)
        self.tree.column("added",  width=90,  anchor="center", minwidth=75)
